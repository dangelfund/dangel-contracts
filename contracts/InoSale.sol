...
/**
 * Users can purchase NFTs after sale started and claim after sale ended
 */

contract INOSale is Ownable, Pausable, AccessControl,  ReentrancyGuard {
    ...
    // The min cap amount each user can purchase IDO up to
    uint256 public purchaseMinCap;
    // The max cap amount each user can purchase IDO up to
    uint256 public purchaseMaxCap;
    ...
    event PurchaseMinCapChanged(uint256 purchaseMinCap);
    event PurchaseMaxCapChanged(uint256 purchaseMaxCap);
    ...
    constructor(
        ...
        uint256 _idoPrice,
        uint256 _purchaseMinCap,
        uint256 _purchaseMaxCap,
        uint256 _startTime,
        uint256 _endTime
    ) {
        require(address(_purchaseToken) != address(0), "PURCHASE_TOKEN_ADDRESS_INVALID");
        require(_idoPrice > 0, "TOKEN_PRICE_INVALID");
        require(_purchaseMinCap > 0, "PURCHASE_MIN_CAP_INVALID");
        require(_purchaseMaxCap > 0, "PURCHASE_MAX_CAP_INVALID");
        ...
        idoPrice = _idoPrice;
        purchaseMinCap = _purchaseMinCap;
        purchaseMaxCap = _purchaseMaxCap;
     ...
     */
    function setPurchaseMinCap(uint256 _purchaseMinCap) external onlyOwner {
        purchaseMinCap = _purchaseMinCap;

        emit PurchaseMinCapChanged(_purchaseMinCap);
    }

    /**
     * @dev Set purchase cap for each user
     */
    function setPurchaseMaxCap(uint256 _purchaseMaxCap) external onlyOwner {
        purchaseMaxCap = _purchaseMaxCap;

        emit PurchaseMaxCapChanged(_purchaseMaxCap);
    }
    ...
    function purchase(uint256 amount) external nonReentrant whenNotPaused {
        ...
        require(purchasedAmounts[_msgSender()] + amount >= purchaseMinCap, "PURCHASE_MIN_CAP_EXCEEDED");
        require(purchasedAmounts[_msgSender()] + amount <= purchaseMaxCap, "PURCHASE_MAX_CAP_EXCEEDED");
